'''在jupyter notebook中绘制经典Turtle图像。

使用Ipython的html功能绘制svg形状。

图像风格受 Blockly Games / Turtle启发(blockly-games.appspot.com/turtle)。'''
...
def make_turtle(speed=DEFAULT_SPEED, width=DEFAULT_WINDOW_SIZE[0], height=DEFAULT_WINDOW_SIZE[1]):
    ...
    if isinstance(speed,int) == False or speed not in range(1, 14):
        raise ValueError('speed 必须是[1,13]区间中的整数。')
    turtle_speed = speed
    ...
    if not (isinstance(window_size, tuple) and
        ...
        raise ValueError('window_size 必须是两个整数的元组。')
...
def _updateDrawing():
    if drawing_window == None:
        raise AttributeError("显示未初始化。在使用前调用 make_turtle() 。")
...
def right(degrees):
    ...
    if not isinstance(degrees, (int,float)):
        raise ValueError('degrees 必须是一个数。')
...
def setx(x):
    if not isinstance(x, (int,float)):
        raise ValueError('新的x坐标必须是一个数。')
    if x < 0:
        raise ValueError('新的x坐标必须非负。')
...
def sety(y):
    if not isinstance(y, (int,float)):
        raise ValueError('新的y坐标必须是一个数。')
    if y < 0:
        raise ValueError('新的y坐标必须非负。')
...
def _processColor(color):
    if isinstance(color, str):
        ...
        if not _validateColorString(color):
            raise ValueError('提供的color无效。可以是预设的html颜色名，3-6位hex字符串或者rgb字符串。')
        return color
    elif isinstance(color, tuple):
        if not _validateColorTuple(color):
            raise ValueError('提供的color元组无效。必须是3个整数的元组。取值范围为[0,255]。')
        return 'rgb(' + str(color[0]) + ',' + str(color[1]) + ',' + str(color[2]) + ')'
    else:
        raise ValueError('第1个参数必须是颜色字符串或者元组。')
...
def bgcolor(color = None, c2 = None, c3 = None):
    ...
    if color is None:
        ...
        if c3 is None:
            raise ValueError('如果设置了第2个参数，也必须设置第3个参数，不全rgb集合。')
        color = (color, c2, c3)
...
def width(width = None):
    ...
    if width is None:
        ...
        if not isinstance(width, int):
            raise ValueError('新的width参数必须为整数。')
        if not width > 0:
            raise ValueError('新的width参数必须为正数。')
...
def write(obj, **kwargs):
    ...
    if "font" in kwargs:
        ...
        if len(font) != 3 or isinstance(font[0], int) == False or isinstance(font[1], str) == False or font[2] not in {'bold','italic','underline','normal'}:
            raise ValueError('font参数必须由字体尺寸（int），字体族（str）和字体类型三元组组成。字体类型可以是{bold, italic, underline, normal}之一。')
        font_size = font[0]
        font_family = font[1]
        font_type = font[2]
...
def shape(shape=None):
    ...
    if shape is None:
        ...
        raise ValueError('提供的shape参数无效。有效的选项为: ' + str(VALID_TURTLE_SHAPES))
